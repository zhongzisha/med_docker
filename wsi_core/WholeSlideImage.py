...
from wsi_core.wsi_utils import savePatchIter_bag_hdf5, initialize_hdf5_bag, coord_generator, save_hdf5, sample_indices, \
    screen_coords, isBlackPatch, isWhitePatch, to_percentiles, load_pkl, save_pkl
import itertools
...
class WholeSlideImage(object):
    ...
    def process_contours_modified(self, save_path, patch_level=0, patch_size=256, step_size=256, **kwargs):
        save_path_hdf5 = os.path.join(save_path, str(self.name) + '.h5')
        print("Creating patches for: ", self.name, "...", )
        elapsed = time.time()
        n_contours = len(self.contours_tissue)
        ...
        for idx, cont in enumerate(self.contours_tissue):
            ...
            asset_dict, attr_dict = self.process_contour_modified(cont, self.holes_tissue[idx], patch_level, save_path,
                                                                  patch_size, step_size, **kwargs)
            if len(asset_dict) > 0:
                if init:
                    save_hdf5(save_path_hdf5, asset_dict, attr_dict, mode='w')
        ...
        if os.path.exists(save_path_hdf5):
            file = h5py.File(save_path_hdf5, 'r')
            dset = file['coords']
            coords = dset[:]
            file.close()

            save_path_pkl = os.path.join(save_path, str(self.name) + '.pkl')
            save_pkl(save_path_pkl, {'patch_loc': coords,
                                     'contours_tissue': self.contours_tissue,
                                     'holes_tissue': self.holes_tissue})

        return self.hdf5_file

    def process_contour_modified(self, cont, contour_holes, patch_level, save_path, patch_size=256, step_size=256,
                                 contour_fn='four_pt', use_padding=True, top_left=None, bot_right=None):
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (
            0, 0, self.level_dim[patch_level][0], self.level_dim[patch_level][1])
